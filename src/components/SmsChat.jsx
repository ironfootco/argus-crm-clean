...
export default function SmsChat({ customerId, customerPhone }) {
  ...
  const safeCustomerPhone = React.useMemo(() => {
    ...
    if (typeof customerPhone === 'object') {
      return String(customerPhone.phone || customerPhone.number || '');
    }
    return String(customerPhone);
  }, [customerPhone]);

  const safeCustomerId = React.useMemo(() => {
    if (!customerId) return null;
    if (typeof customerId === 'object') {
      return customerId.id || customerId.customerId || null;
    }
    return customerId;
  }, [customerId]);
  ...
  const fetchMessages = async (showLoading = true) => {
    try {
      ...
      const digits = safeCustomerPhone.replace(/\D/g, '');
      ...
      const phoneFormats = [`+1${coreNumber}`, `+${coreNumber}`, coreNumber, `1${coreNumber}`, safeCustomerPhone];
  ...
  const handleClickToCall = async () => {
    ...
    const cCore = safeCustomerPhone.replace(/\D/g, '').slice(-10);
  ...
  const sendOutbound = async (text, mediaUrl = null) => {
    const coreNumber = safeCustomerPhone.replace(/\D/g, '').slice(-10);
  ...
  const formatDate = (dStr) => {
    try { 
      if (!dStr) return '';
      return new Date(dStr).toLocaleDateString([], { month: 'short', day: 'numeric' }); 
    } catch (e) { return ''; }
  };
      ...
      <div style={{ flex: 1, padding: 20, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
        {messages.length === 0 ? (
          ...
          messages.map((msg, idx) => {
            ...
            return (
              <div key={msg.id || idx} style={{ display: 'flex', flexDirection: 'column', alignItems: isOutbound ? 'flex-end' : 'flex-start' }}>
                <div style={{ maxWidth: '85%', padding: '10px 14px', borderRadius: 12, fontSize: 14, lineHeight: '1.4', background: bgColor, color: textColor, border: isOutbound ? 'none' : '1px solid var(--border-color)', borderBottomRightRadius: isOutbound ? 2 : 12, borderBottomLeftRadius: isOutbound ? 12 : 2 }}>
                  ...
                  {mediaUrl && (
                    mediaUrl.includes('Recordings') ? 
                      <audio controls src={mediaUrl} style={{ width: '100%', maxWidth: '250px', height: '35px', marginTop: '10px', borderRadius: '4px' }} />
                    : 
                      <a href={mediaUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'block', marginTop: bodyText ? '10px' : '0' }}>
                        <img src={mediaUrl} alt="Media" style={{ width: '100%', maxWidth: '250px', borderRadius: '8px', border: '1px solid rgba(0,0,0,0.2)' }} />
                      </a>
                  )}
                </div>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
                  {isOutbound && <span style={{ fontWeight: 'bold' }}>{senderName}</span>}
