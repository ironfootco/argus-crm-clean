      ...
      <div style={{ background: 'var(--bg-card)', border: '2px solid var(--border-color)', borderRadius: 12, width: '100%', maxWidth: 500, marginTop: 'auto', marginBottom: 'auto', padding: 16, color: 'var(--text-main)', boxSizing: 'border-box' }}>
        ...
        <form onSubmit={handleSaveLead} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {selectedCustomerId ? (
            ...
            <>
              <input ref={streetInputRef} placeholder="🔍 Type Street Address (Google Autocomplete)" value={street} onChange={e => setStreet(e.target.value)} style={{ padding: 10, borderRadius: 6, border: '1.5px solid var(--border-color)', background: 'var(--bg-input)', color: 'var(--text-main)', fontSize: 15, boxSizing: 'border-box', width: '100%' }} />
...
function Dashboard({ refreshTrigger, activeWorker }) {
  ...
  const commitStageUpdate = async (job, stage, isPaused = false) => {
    ...
    if ((isPaused && stage === 'On Site / In Progress') || stage === 'Job Complete') {
      if (job.job_started_at) {
        ...
        const newLog = { worker_name: activeWorker, hours: hoursWorked, rate: activeRate };
        updateData.time_logs = [...(job.time_logs || []), newLog];
  ...
  const getNext7Days = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push({ isoStr, dayLabel: i === 0 ? 'Today' : i === 1 ? 'Tomorrow' : d.toLocaleDateString('en-US', { weekday: 'short' }), monthDay: `${d.getMonth() + 1}/${d.getDate()}`, jobCount: jobs.filter(j => j.scheduled_date === isoStr).length });
    }
    return days;
  };
  ...
  const loadoutTitle = isTodayLoadout ? `🚛 ${activeWorker}'s Truck & Tool Loadout` : `🚛 Tool & Equipment Prep for ${formatDate(targetLoadoutDate)}`;
  ...
  return (
    <div>
      ...
      <div style={{ background: 'var(--bg-card)', padding: 18, borderRadius: 8, marginBottom: 20, border: '2px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            {activeShift && <span style={{ fontSize: 13, color: 'var(--success)', fontWeight: 'bold' }}>🟢 Clocked in since {new Date(activeShift.clock_in).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>}
          </div>
        </div>
      ...
      <div style={{ background: 'var(--bg-card)', padding: 18, borderRadius: 8, marginBottom: 25, border: '2px solid var(--border-color)' }}>
        ...
        <div style={{ fontSize: 14, color: 'var(--text-main)' }}>{loadoutMaterials || (isTodayLoadout ? "No materials specified for today's jobs." : `No materials specified for ${formatDate(targetLoadoutDate)}.`)}</div>
      </div>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 }}>
        <h3 style={{ color: 'var(--text-main)', margin: 0 }}>⚡ {activeWorker}&apos;s Schedule ({filteredJobs.length})</h3>
        <span style={{ fontSize: 12, color: 'var(--text-accent)', fontWeight: 'bold' }}>{selectedFilterDate === 'ALL_UPCOMING' ? 'Viewing All Upcoming Days' : `Filtering: ${formatDate(selectedFilterDate)}`}</span>
      </div>
        ...
        {filteredJobs.map(job => {
          ...
          return (
            <React.Fragment key={job.id}>
              {showDateBanner && (
                <div style={{ margin: '15px 0 5px 0', padding: '10px 14px', background: isJobToday ? 'var(--primary)' : 'var(--bg-card)', color: isJobToday ? 'var(--primary-text)' : 'var(--text-accent)', borderRadius: 6, border: '1.5px solid var(--border-color)', fontWeight: 'bold', fontSize: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
