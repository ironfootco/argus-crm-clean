export default async function handler(req, res) {
  ...
  // Standardize phone to 10 pure digits to prevent Wave duplicates
  ...
  if (email && email.trim()) customerInput.email = email.trim();
  if (cleanPhone) customerInput.phone = cleanPhone;

  // Optional: Parse basic address mapping if provided
  ...
  try {
    const response = await fetch('https://gql.waveapps.com/graphql/public', {
      ...
      body: JSON.stringify({
        query: `mutation ($input: CustomerCreateInput!) { customerCreate(input: $input) { didSucceed customer { id } } }`,
